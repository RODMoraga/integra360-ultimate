<template>
        ...
        <div class="p-5 lg:p-6 max-w-screen-2xl mx-auto w-full space-y-5">
          ...
          <div class="d-flex flex-wrap align-items-center justify-content-between gap-3">
            <div>
              <h1 class="h4 fw-bold mb-1 text-dark">
                <i class="fa-solid fa-file-invoice me-2 text-brick-ember"></i>Gestión de Documentos
              </h1>
              <p class="text-secondary small mb-0">Administra documentos con detalle dinámico y filtros de búsqueda avanzados.</p>
            </div>
          ...
          <div class="bg-white rounded-4 shadow-sm p-4">
            <div v-if="isLoading" class="d-flex flex-column align-items-center justify-content-center py-5 gap-3">
              ...
              <p class="text-secondary small">Cargando documentos...</p>
            </div>
  ...
  <div class="modal fade" id="documentViewModal" tabindex="-1" aria-labelledby="documentViewModalLabel" aria-hidden="true" ref="viewModalRef">
    <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
      <div class="modal-content rounded-4 shadow" v-if="selectedDocument">
        <div class="modal-header border-0">
          <h5 class="modal-title fw-bold" id="documentViewModalLabel">
            <i class="fa-solid fa-file-invoice me-2 text-info"></i>
            Detalle de Documento {{ selectedDocument.document_number_label }}
          </h5>
        ...
        <div class="modal-body pt-0">
          <div class="row g-3 mb-3">
            <div class="col-md-4"><DetailRow icon="fa-file-lines" label="Tipo" :value="selectedDocument.document_type_name" /></div>
            <div class="col-md-4"><DetailRow icon="fa-calendar" label="Fecha" :value="formatDate(selectedDocument.document_date)" /></div>
            <div class="col-md-4"><DetailRow icon="fa-circle-check" label="Estado" :value="statusLabel(selectedDocument.status)" /></div>
            <div class="col-md-4"><DetailRow icon="fa-user-group" label="Cliente" :value="selectedDocument.customer_name" /></div>
            <div class="col-md-4"><DetailRow icon="fa-truck-field" label="Proveedor" :value="selectedDocument.supplier_name" /></div>
            <div class="col-md-4"><DetailRow icon="fa-warehouse" label="Bodega" :value="selectedDocument.warehouse_name" /></div>
          </div>
          ...
          <div class="table-responsive border rounded-3">
            <table class="table table-sm table-striped mb-0 align-middle">
              ...
              <tbody>
                <tr v-for="line in selectedDocument.details ?? []" :key="line.id">
        ...
        <div class="modal-footer border-0 d-flex justify-content-between w-100">
          <div class="text-start">
            <p class="mb-1">Subtotal: <strong>{{ formatMoney(selectedDocument.subtotal) }}</strong></p>
            <p class="mb-1">Descuento: <strong>{{ formatMoney(selectedDocument.discount_total) }}</strong></p>
            <p class="mb-0">Impuesto: <strong>{{ formatMoney(selectedDocument.tax_total) }}</strong></p>
          </div>
          <div class="text-end">
            <p class="mb-0 fs-5">Total: <strong class="text-primary">{{ formatMoney(selectedDocument.total) }}</strong></p>
          </div>
        </div>
      </div>
    </div>
...
const draftFilters = reactive<DocumentListFilters>({
  partner_name: "",
  status: "",
...
const appliedFilters = reactive<DocumentListFilters>({
  partner_name: "",
  status: "",
  date_from: defaultStartDate,
  date_to: defaultEndDate
...
const { data, isLoading, isError } = useDocuments(appliedFilters);
...
const shouldShowCustomerSelect = computed(() => {
  const scope = selectedDocumentType.value?.counterpart_scope;
...
watch(
  () => selectedDocumentType.value?.counterpart_scope,
  (scope) => {
    if (scope === "CUSTOMER") {
      form.supplier_id = "";
    }
...
async function openEditModal(id: string) {
  const full = await documentService.getById(id);
...
async function openViewModal(id: string) {
  selectedDocument.value = await documentService.getById(id);
...
async function submitForm() {
  try {
    ...
    if (isEditMode.value && editingId.value) {
      await updateDocument({ id: editingId.value, payload });
      await Swal.fire({ icon: "success", title: "Documento actualizado", timer: 1500, showConfirmButton: false });
    } else {
      await createDocument(payload as { document_type_id: number; details: NonNullable<typeof payload.details> });
      await Swal.fire({ icon: "success", title: "Documento creado", timer: 1500, showConfirmButton: false });
    }
...
async function removeDocument(id: string) {
  const result = await Swal.fire({
    ...
    text: "El documento se eliminará lógicamente y ya no aparecerá en listados.",
  ...
  try {
    ...
    await Swal.fire({ icon: "success", title: "Documento eliminado", timer: 1500, showConfirmButton: false });
  } catch (error: unknown) {
...
const tableRows = computed(() => documents.value.map((row) => ({
  id: row.id,
...
const tableColumns = [
  ...
  { data: "document_date" },
  { data: "partner_name" },
  { data: "status", render: (value: DocumentStatus) => statusBadge(value) },
  { data: "total" },
  { data: "details_count" },
  {
    data: null,
    ...
    render: (_: unknown, __: unknown, row: { id: string }) => `
      <div class="btn-group" role="group" aria-label="Acciones de fila">
        ...
        <button type="button" class="btn btn-sm btn-outline-warning" data-action="edit" data-id="${row.id}" title="Editar documento" aria-label="Editar documento"><i class="fa-solid fa-pen-to-square"></i></button>
        <button type="button" class="btn btn-sm btn-outline-danger" data-action="delete" data-id="${row.id}" title="Eliminar documento" aria-label="Eliminar documento"><i class="fa-solid fa-trash"></i></button>
      </div>
    `
  }
];
...
const tableRenderKey = computed(() => `${documents.value.length}-${appliedFilters.partner_name ?? ''}-${appliedFilters.status ?? ''}-${appliedFilters.date_from ?? ''}-${appliedFilters.date_to ?? ''}`);
